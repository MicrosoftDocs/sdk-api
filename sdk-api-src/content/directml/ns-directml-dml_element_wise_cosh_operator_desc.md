---
UID: NS:directml.DML_ELEMENT_WISE_COSH_OPERATOR_DESC
title: DML_ELEMENT_WISE_COSH_OPERATOR_DESC
description: Describes a DirectML trigonometric operator that performs the element-wise hyperbolic cosine function f(x) = ((e^x + e^-x) / 2) * scale + bias, where the scale and bias terms are optional.
tech.root: directml
ms.date: 01/30/2020
targetos: Windows
...
f1_keywords:
 - directml/DML_ELEMENT_WISE_COSH_OPERATOR_DESC
dev_langs:
 - c++
---
...
Describes a DirectML trigonometric operator that performs the element-wise hyperbolic cosine function f(x) = ((e^x + e^-x) / 2) * scale + bias, where the scale and bias terms are optional.
