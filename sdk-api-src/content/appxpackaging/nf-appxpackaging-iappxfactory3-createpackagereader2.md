...
ms.date: 02/13/2023
targetos: Windows
description: Creates a read-only package reader from the contents provided by an IStream, with an optional parameter for specifying the expected digest for the manifest.
prerelease: true
req.assembly: 
...
Creates a read-only package reader from the contents provided by an [IStream](../objidl/nn-objidl-istream.md), with an optional parameter for specifying the expected digest for the manifest. This method does not validate the digital signature.
