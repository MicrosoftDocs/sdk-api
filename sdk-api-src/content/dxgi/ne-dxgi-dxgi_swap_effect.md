...
The difference between <b>DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL</b> and <b>DXGI_SWAP_EFFECT_FLIP_DISCARD</b> is that <b>DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL</b> forces DXGI to guarantee that the value of each back buffer is preserved across `IDXGISwapChain::Present` calls, whereas <b>DXGI_SWAP_EFFECT_FLIP_DISCARD</b> doesn't provide this guarantee. This can allow the compositor, under certain scenarios, to use the application's back buffer as the entire display back buffer, even when the application is not the only visible item on the screen, so it can draw other content on top of the back buffer. This optimisation can occur with both <b>DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL</b> and <b>DXGI_SWAP_EFFECT_FLIP_DISCARD</b> models when the application is the only item visible on screen. However, using <b>DXGI_SWAP_EFFECT_FLIP_DISCARD</b> forces the app to redraw every single frame, which can take additional processing power and can increase power consumption.
