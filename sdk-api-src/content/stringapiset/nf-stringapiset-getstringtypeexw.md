...
> [!IMPORTANT]
> Using the **GetStringTypeEx** function incorrectly can compromise the security of your application. To avoid a buffer overflow, the application must set the output buffer size correctly. For more security information, see [Security Considerations: Windows User Interface](/windows/desktop/AppUIStart/sec-ui).

Unlike its close relatives [GetStringTypeA](/windows/desktop/api/winnls/nf-winnls-getstringtypea) and [GetStringTypeW](/windows/desktop/api/stringapiset/nf-stringapiset-getstringtypew), this function exhibits appropriate ANSI or Unicode behavior through the use of the #define UNICODE switch. This is the recommended function for character type retrieval.
