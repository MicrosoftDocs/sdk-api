---
UID: NE:p2p.peer_node_change_type_tag
title: peer_node_change_type_tag
author: windows-sdk-content
description: The PEER_NODE_CHANGE_TYPE enumeration specifies the types of peer node graph status changes.
old-location: p2p\peer_node_change_type.htm
old-project: P2PSdk
ms.assetid: e14e432a-5edb-4bf1-9c64-e5ece5b9254e
ms.author: windowssdkdev
ms.date: 05/08/2018
ms.keywords: PEER_NODE_CHANGE_CONNECTED, PEER_NODE_CHANGE_DISCONNECTED, PEER_NODE_CHANGE_TYPE, PEER_NODE_CHANGE_TYPE enumeration [Peer Networking], PEER_NODE_CHANGE_UPDATED, p2p.peer_node_change_type, p2p/PEER_NODE_CHANGE_CONNECTED, p2p/PEER_NODE_CHANGE_DISCONNECTED, p2p/PEER_NODE_CHANGE_TYPE, p2p/PEER_NODE_CHANGE_UPDATED, peer_node_change_type_tag
ms.prod: windows
ms.technology: windows-sdk
ms.topic: enum
req.header: p2p.h
req.include-header: 
req.target-type: Windows
req.target-min-winverclnt: Windows XP with SP2 [desktop apps only],Windows XP with SP1 with the Advanced Networking Pack forWindows XP
req.target-min-winversvr: None supported
req.kmdf-ver: 
req.umdf-ver: 
req.ddi-compliance: 
req.unicode-ansi: 
req.idl: 
req.max-support: 
req.namespace: 
req.assembly: 
req.type-library: 
req.typenames: PEER_NODE_CHANGE_TYPE
topic_type:
-	APIRef
-	kbSyntax
api_type:
-	HeaderDef
api_location:
-	P2P.h
api_name:
-	PEER_NODE_CHANGE_TYPE
product: Windows
targetos: Windows
req.lib: 
req.dll: 
req.irql: 
req.product: Rights Management Services client 1.0 or later
---

# peer_node_change_type_tag enumeration


## -description


The <b>PEER_NODE_CHANGE_TYPE</b> enumeration specifies the types of peer node graph status changes.


## -enum-fields




### -field PEER_NODE_CHANGE_CONNECTED

The peer node has connected to the graph.


### -field PEER_NODE_CHANGE_DISCONNECTED

The peer node has disconnected from the graph.


### -field PEER_NODE_CHANGE_UPDATED

The peer node's status within the graph has changed.

