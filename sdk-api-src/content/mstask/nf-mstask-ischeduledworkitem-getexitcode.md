...
Retrieves the last exit code returned by the executable associated with the <a href="/windows/desktop/api/mstask/ns-mstask-_monthlydow">work item</a> on its last run. The method also returns the exit code returned to Task Scheduler when it last attempted to run the work item.
