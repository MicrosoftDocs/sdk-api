---
UID: NF:taskschd.ITaskService.get_HighestVersion
title: ITaskService::get_HighestVersion (taskschd.h)
description: Indicates the highest version of Task Scheduler that a computer supports.
helpviewer_keywords: ["HighestVersion property [Task Scheduler]","HighestVersion property [Task Scheduler]","ITaskService interface","HighestVersion property [Task Scheduler]","TBD object","ITaskService interface [Task Scheduler]","HighestVersion property","ITaskService.HighestVersion","ITaskService.get_HighestVersion","ITaskService::HighestVersion","ITaskService::get_HighestVersion","TBD object [Task Scheduler]","HighestVersion property","get_HighestVersion","taskschd.itaskservice_highestversion","taskschd/ITaskService::HighestVersion","taskschd/ITaskService::get_HighestVersion"]
old-location: taskschd\itaskservice_highestversion.htm
tech.root: taskschd
ms.assetid: 27b08a17-fdd9-4c28-b498-e10e80ae49f0
ms.date: 12/05/2018
ms.keywords: HighestVersion property [Task Scheduler], HighestVersion property [Task Scheduler],ITaskService interface, HighestVersion property [Task Scheduler],TBD object, ITaskService interface [Task Scheduler],HighestVersion property, ITaskService.HighestVersion, ITaskService.get_HighestVersion, ITaskService::HighestVersion, ITaskService::get_HighestVersion, TBD object [Task Scheduler],HighestVersion property, get_HighestVersion, taskschd.itaskservice_highestversion, taskschd/ITaskService::HighestVersion, taskschd/ITaskService::get_HighestVersion
req.header: taskschd.h
req.include-header: 
req.target-type: Windows
req.target-min-winverclnt: Windows Vista [desktop apps only]
req.target-min-winversvr: Windows Server 2008 [desktop apps only]
req.kmdf-ver: 
req.umdf-ver: 
req.ddi-compliance: 
req.unicode-ansi: 
req.idl: 
req.max-support: 
req.namespace: 
req.assembly: 
req.type-library: 
req.lib: Taskschd.lib
req.dll: Taskschd.dll
req.irql: 
targetos: Windows
req.typenames: 
req.redist: 
ms.custom: 19H1
f1_keywords:
 - ITaskService::get_HighestVersion
 - taskschd/ITaskService::get_HighestVersion
dev_langs:
 - c++
topic_type:
 - APIRef
 - kbSyntax
api_type:
 - COM
api_location:
 - taskschd.dll
api_name:
 - ITaskService::get_HighestVersion
---

# ITaskService::get_HighestVersion


## -description

Indicates the highest version of Task Scheduler that a computer supports.

This property is read-only.

## -parameters

## -see-also

<a href="/windows/desktop/api/taskschd/nn-taskschd-itaskservice">ITaskService</a>

