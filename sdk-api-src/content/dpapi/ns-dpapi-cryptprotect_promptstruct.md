...
> [!IMPORTANT]
> **Deprecated — removal date: April 2027.** The interactive prompt-based encryption and decryption flow provided by this structure (`CRYPTPROTECT_PROMPT_ON_PROTECT` and `CRYPTPROTECT_PROMPT_ON_UNPROTECT`) will be removed in April 2027. Applications that rely on prompt-based flows must migrate before that date; after removal, decryption attempts that depend on those flows will fail and the data will no longer be recoverable through them.
>
