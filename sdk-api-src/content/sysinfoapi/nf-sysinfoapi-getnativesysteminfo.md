...
<a href="/windows/desktop/api/sysinfoapi/nf-sysinfoapi-getsysteminfo">GetSystemInfo</a> function. If the function is called from an x86 or x64 application running on a system that does not have an Intel64 or x64 processor (such as ARM64), it will return information as if the system is x86 if only x86 emulation is supported, or x64 if x64 emulation is also supported.
...
To determine whether a Win32-based application is running under WOW64, or whether a system does not have an Intel64 or x64 processor, call the 
