...
A handle to a <a href="https://www.bing.com/search?q=device+information+set">device information set</a> that contains a device information element that represents the device for which to create a registry key. 
...
The handle to an open INF file that contains an <a href="https://www.bing.com/search?q=INF+DDInstall+section">INF DDInstall section</a> to be executed for the newly created key. This parameter is optional and can be <b>NULL</b>. If this parameter is specified, <i>InfSectionName</i> must be specified as well.
