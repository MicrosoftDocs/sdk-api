...
Because **CreateIconFromResourceEx** creates the icon or cursor from a memory buffer that has no associated module or resource identifier, the shared-resource cache — which is keyed by module and resource ID — cannot store or return this handle even though the **LR_SHARED** flag is passed. The returned handle is therefore owned by the caller.
