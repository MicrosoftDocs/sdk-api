...
-   **A device-independent bitmap (DIB) image.** The image data begins with a [BITMAPINFOHEADER](/windows/win32/api/wingdi/ns-wingdi-bitmapinfoheader); the legacy **BITMAPCOREHEADER** format is also accepted. For a cursor (*fIcon* = **FALSE**), the image is preceded by the cursor hotspot ([LOCALHEADER](/windows/win32/menurc/localheader)).
