...
If the resource group contains more than one image, the function assigns each candidate a score measuring how far it departs from the target, and returns the entry with the lowest score (a score of zero is an exact match). The target is the requested width and height — or, when a dimension is zero and **LR_DEFAULTSIZE** is set, the corresponding system-metric size (see *cxDesired*) — together with the color depth of the current display device. When both dimensions are zero and **LR_DEFAULTSIZE** is not set, size does not take part in the target and selection is based on color depth alone. The score is the sum of three terms:
