...
In **MAPVK\_VK\_TO\_CHAR** mode [virtual-key codes](/windows/win32/inputdev/virtual-key-codes) for 'A'..'Z' keys are translated to upper-case 'A'..'Z' characters regardless of provided input locale identifier due to historical reasons. Instead, you can use [ToAscii](/windows/win32/api/winuser/nf-winuser-toascii) function if you want to translate virtual-key code to the corresponding character.
