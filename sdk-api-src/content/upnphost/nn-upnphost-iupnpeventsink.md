...
Method that sends the list of <a href="https://msdn.microsoft.com/library/ms221242(v=VS.85).aspx">dispids</a> for state variables that have changed and their changed values to the device host. The device host then queries the device for the changed values and sends the event to all subscribed control points.
