...
other components. This name string will be included within each ETW event
...
using a GUID generated by hashing the provider name. See below for information
about generating the provider ID.
...
a handle that can be used to access it. It also creates a hidden global variable
that will be used to manage the provider and records provider information such
as the provider's name and GUID.
...
> scoped to the module, i.e. the handle can be used as needed within the EXE,
> DLL, or SYS file, but it should not be used outside the scope of the module,
> i.e. it should not be passed to other DLLs in the same process. Each EXE, DLL,
> or SYS file should use its own provider handle and should perform its own
> Register and Unregister. In debug builds, an assertion will fire if you
> attempt to write using a provider handle from another module.
...
An ETW TraceLogging provider is identified by both provider name and provider
ID. Both the name and the ID need to be unique so they do not conflict with
other providers. In addition, the name and the ID should be linked: once a
particular name is used with a particular ID for an ETW provider, that name
should not be used with any other ID and that ID should not be used with any
other name.
...
using a randomly-generated GUID for the provider ID, Microsoft recommends using
a hash of the provider name following an ETW name-hashing system. This provides
several benefits: it's easier to remember just the name; the ID and the name are
automatically linked; tools such as tracelog, traceview, EventSource, and WPR
have special support for providers that use IDs generated using the ETW
name-hashing system.
...
You can use PowerShell to obtain the hash-based provider ID for a particular
provider name:
...
In C#, the provider ID for a name can be generated as follows:
