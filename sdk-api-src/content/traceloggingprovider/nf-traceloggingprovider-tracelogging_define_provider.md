...
description:
  Allocates storage for a TraceLogging provider and creates a handle to the
  provider.
helpviewer_keywords:
  [
...
A handle to a TraceLogging provider (TraceLoggingHProvider) created using
[TRACELOGGING_DECLARE_PROVIDER](./nf-traceloggingprovider-tracelogging_declare_provider.md).

### -param providerName [in]

The name of the TraceLogging provider. This must be a string literal--do not use
a variable.
...
This can be any GUID (for example, one generated using the `guidgen` SDK tool or
[https://uuidgen.org](https://uuidgen.org)). However, Microsoft has established
a convention for generating the GUID using a hash of the provider name. This
provides several benefits: it's easier to remember just the name, and it works
with tools such as tracelog, traceview, EventSource, and WPR.
...
Before using this macro, you need to declare your TraceLogging provider using
[TRACELOGGING_DECLARE_PROVIDER](./nf-traceloggingprovider-tracelogging_declare_provider.md).
Once the provider is created, it is in the unregistered state. Before it can
respond to any write calls, you need to register the provider using
...
If your provider is a part of a group, add the _options_ parameter.
