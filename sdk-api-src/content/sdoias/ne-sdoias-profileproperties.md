---
UID: NE:sdoias._PROFILEPROPERTIES
title: PROFILEPROPERTIES (sdoias.h)
description: The PROFILEPROPERTIES enumeration type enumerates properties associated with a profile.
helpviewer_keywords: ["PROFILEPROPERTIES","PROFILEPROPERTIES enumeration [Network Policy Server]","PROPERTY_PROFILE_ATTRIBUTES_COLLECTION","_sdo_profileproperties","nps.SDO_profileproperties","sdo.profileproperties","sdoias/PROFILEPROPERTIES","sdoias/PROPERTY_PROFILE_ATTRIBUTES_COLLECTION"]
old-location: nps\SDO_profileproperties.htm
tech.root: Nps
ms.assetid: 54a498c4-af27-4438-a78e-ce92c632a122
ms.date: 12/05/2018
ms.keywords: PROFILEPROPERTIES, PROFILEPROPERTIES enumeration [Network Policy Server], PROPERTY_PROFILE_ATTRIBUTES_COLLECTION, _sdo_profileproperties, nps.SDO_profileproperties, sdo.profileproperties, sdoias/PROFILEPROPERTIES, sdoias/PROPERTY_PROFILE_ATTRIBUTES_COLLECTION
req.header: sdoias.h
req.include-header: 
req.target-type: Windows
req.target-min-winverclnt: None supported
req.target-min-winversvr: Windows Server 2008
req.kmdf-ver: 
req.umdf-ver: 
req.ddi-compliance: 
req.unicode-ansi: 
req.idl: SdoIas.idl
req.max-support: 
req.namespace: 
req.assembly: 
req.type-library: 
req.lib: 
req.dll: 
req.irql: 
targetos: Windows
req.typenames: PROFILEPROPERTIES
req.redist: 
ms.custom: 19H1
f1_keywords:
 - _PROFILEPROPERTIES
 - sdoias/_PROFILEPROPERTIES
 - PROFILEPROPERTIES
 - sdoias/PROFILEPROPERTIES
dev_langs:
 - c++
topic_type:
 - APIRef
 - kbSyntax
api_type:
 - HeaderDef
api_location:
 - SdoIas.h
api_name:
 - _PROFILEPROPERTIES
 - PROFILEPROPERTIES
---

# PROFILEPROPERTIES enumeration


## -description

The 
<b>PROFILEPROPERTIES</b> enumeration type enumerates properties associated with a profile.

## -enum-fields

### -field PROPERTY_PROFILE_ATTRIBUTES_COLLECTION

The attributes associated with the profile.

### -field PROPERTY_PROFILE_IPFILTER_TEMPLATE_GUID

## -see-also

<a href="/windows/desktop/api/sdoias/ne-sdoias-iasproperties">IASPROPERTIES</a>



<a href="/windows/desktop/api/sdoias/ne-sdoias-napproperties">NAPPROPERTIES</a>

