...
Determine the amount of data that can be read atomically from socket <i>s</i>. The <i>lpvOutBuffer</i> parameter points at an **unsigned long** in which [**WSAIoctl**](https://docs.microsoft.com/windows/win32/api/winsock2/nf-winsock2-wsaioctl) stores the result.
...
Request a runtime reservation for a block of TCP or UDP ports. For runtime port reservations, the port pool requires that reservations be consumed from the process on whose socket the reservation was granted. Runtime port reservations last only as long as the lifetime of the socket on which the [**SIO_ACQUIRE_PORT_RESERVATION**](https://docs.microsoft.com/windows/win32/winsock/sio-acquire-port-reservation) IOCTL was called. In contrast, persistent port reservations created using the [**CreatePersistentTcpPortReservation**](https://docs.microsoft.com/windows/win32/api/iphlpapi/nf-iphlpapi-createpersistenttcpportreservation) or [**CreatePersistentUdpPortReservation**](https://docs.microsoft.com/windows/win32/api/iphlpapi/nf-iphlpapi-createpersistentudpportreservation) function may be consumed by any process with the ability to obtain persistent reservations.

For more detailed information, see the [**SIO_ACQUIRE_PORT_RESERVATION**](https://docs.microsoft.com/windows/win32/winsock/sio-acquire-port-reservation) reference.

[**SIO_ACQUIRE_PORT_RESERVATION**](https://docs.microsoft.com/windows/win32/winsock/sio-acquire-port-reservation) is supported on Windows Vista and later versions of the operating system.
...
Associate a socket with a persistent or runtime reservation for a block of TCP or UDP ports identified by the port reservation token. The [**SIO_ASSOCIATE_PORT_RESERVATION**](https://docs.microsoft.com/windows/win32/winsock/sio-associate-port-reservation) IOCTL must be issued before the socket is bound. If and when the socket is bound, the port assigned to it will be selected from the port reservation identified by the given token. If no ports are available from the specified reservation, the <b><a href="/windows/win32/api/winsock/nf-winsock-bind">Bind</a></b> function call will fail.

For more detailed information, see the [**SIO_ASSOCIATE_PORT_RESERVATION**](https://docs.microsoft.com/windows/win32/winsock/sio-associate-port-reservation) reference.

[**SIO_ASSOCIATE_PORT_RESERVATION**](https://docs.microsoft.com/windows/win32/winsock/sio-associate-port-reservation) is supported on Windows Vista and later versions of the operating system.
...
If the output buffer is not large enough for a socket handle (the <i>cbOutBuffer</i> is less than the size of a **SOCKET**) or the <i>lpvOutBuffer</i> parameter is a **NULL** pointer, **SOCKET_ERROR** is returned as the result of this IOCTL and [**WSAGetLastError**](https://docs.microsoft.com/windows/win32/api/winsock/nf-winsock-wsagetlasterror) returns [WSAEFAULT](/windows/win32/winsock/windows-sockets-error-codes-2#wsaefault).
...
Retrieves the base service provider handle for a socket used by the [**select**](https://docs.microsoft.com/sql/t-sql/queries/select-transact-sql?view=sql-server-ver15) function. The returned value is a **SOCKET**.

This Ioctl is used by a layered service provider to ensure the provider intercept the [**select**](https://docs.microsoft.com/sql/t-sql/queries/select-transact-sql?view=sql-server-ver15) function.

If the output buffer is not large enough for a socket handle (the <i>cbOutBuffer</i> is less than the size of a **SOCKET**) or the <i>lpvOutBuffer</i> parameter is a **NULL** pointer, **SOCKET_ERROR** is returned as the result of this IOCTL and [**WSAGetLastError**](https://docs.microsoft.com/windows/win32/api/winsock/nf-winsock-wsagethttps://docs.microsoft.com/en-us/windows/win32/api/winsock2/nf-winsock2-wsapolllasterror) returns [WSAEFAULT](/windows/win32/winsock/windows-sockets-error-codes-2#wsaefault).
...
Retrieves the base service provider handle for a socket used by the [**WSAPoll**](https://docs.microsoft.com/windows/win32/api/winsock2/nf-winsock2-wsapoll) function. The <i>lpOverlapped</i> parameter must be a **NULL** pointer. The returned value is a **SOCKET**.

This Ioctl is used by a layered service provider to ensure the provider intercept the [**WSAPoll**](https://docs.microsoft.com/windows/win32/api/winsock2/nf-winsock2-wsapoll) function.

If the output buffer is not large enough for a socket handle (the <i>cbOutBuffer</i> is less than the size of a **SOCKET**), the <i>lpvOutBuffer</i> parameter is a **NULL** pointer, or the <i>lpOverlapped</i> parameter is not a **NULL** pointer, **SOCKET_ERROR** is returned as the result of this IOCTL and [**WSAGetLastError**](https://docs.microsoft.com/windows/win32/api/winsock/nf-winsock-wsagetlasterror) returns [WSAEFAULT](/windows/win32/winsock/windows-sockets-error-codes-2#wsaefault).
...
Returns a list of configured IP interfaces and their parameters as an array of [**INTERFACE_INFO**](https://docs.microsoft.com/windows/win32/api/ws2ipdef/ns-ws2ipdef-interface_info) structures.
...
The ISB value per connection is available from the TCP protocol implementation in Windows Server 2008, Windows Vista with Service Pack 1 (SP1), and later versions of the operating system. The [**SIO_IDEAL_SEND_BACKLOG_CHANGE**](https://docs.microsoft.com/windows/win32/winsock/sio-ideal-send-backlog-change) IOCTL can be used by an application to get notification when the ISB value changes dynamically for a connection.

For more detailed information, see the **SIO_IDEAL_SEND_BACKLOG_CHANGE** reference.

[**SIO_IDEAL_SEND_BACKLOG_CHANGE**](https://docs.microsoft.com/windows/win32/winsock/sio-ideal-send-backlog-change) is supported on Windows Server 2008, Windows Vista with SP1, and later versions of the operating system.
...
For more detailed information, see the [**SIO_IDEAL_SEND_BACKLOG_QUERY**](https://docs.microsoft.com/windows/win32/winsock/sio-ideal-send-backlog-query) reference.

[**SIO_IDEAL_SEND_BACKLOG_QUERY**](https://docs.microsoft.com/windows/win32/winsock/sio-ideal-send-backlog-query) is supported on Windows Server 2008, Windows Vista with SP1, and later versions of the operating system.
...
Enables a socket to receive all IPv4 or IPv6 packets passing throuigh a network interface. The socket handle passed to the [**WSAIoctl**](https://docs.microsoft.com/windows/win32/api/winsock2/nf-winsock2-wsaioctl) function must be one of the following:
...
For more detailed information, see the [**SIO_RCVALL**](https://docs.microsoft.com/windows/win32/winsock/sio-rcvall) reference.
...
Releases a runtime reservation for a block of TCP or UDP ports. The runtime reservation to be released must have been obtained from the issuing process using the [**SIO_ACQUIRE_PORT_RESERVATION**](https://docs.microsoft.com/windows/win32/winsock/sio-acquire-port-reservation) IOCTL.

For more detailed information, see the [**SIO_RELEASE_PORT_RESERVATION**](https://docs.microsoft.com/windows/win32/winsock/sio-release-port-reservation) reference.

[**SIO_RELEASE_PORT_RESERVATION**](https://docs.microsoft.com/windows/win32/winsock/sio-release-port-reservation) is supported on Windows Vista and later versions of the operating system.
...
If output buffer is not large enough to contain the interface address, SOCKET_ERROR is returned as the result of this IOCTL and [**WSAGetLastError**](https://docs.microsoft.com/windows/win32/api/winsock/nf-winsock-wsagetlasterror) returns [WSAEFAULT](/windows/win32/winsock/windows-sockets-error-codes-2#wsaefault). The required size of the output buffer will be returned in <i>lpcbBytesReturned</i> in this case. Note the WSAEFAULT error code is also returned if the <i>lpvInBuffer</i>, <i>lpvOutBuffer</i>, or <i>lpcbBytesReturned</i> parameter is not totally contained in a valid part of the user address space.

If the destination address specified in the input buffer cannot be reached through any of the available interfaces, SOCKET_ERROR is returned as the result of this IOCTL and [**WSAGetLastError**](https://docs.microsoft.com/windows/win32/api/winsock/nf-winsock-wsagetlasterror) returns [WSAENETUNREACH](/windows/win32/winsock/windows-sockets-error-codes-2#wsaenetunreach) or even [WSAENETDOWN](/windows/win32/winsock/windows-sockets-error-codes-2#wsaenetdown) if all of the network connectivity is lost.
