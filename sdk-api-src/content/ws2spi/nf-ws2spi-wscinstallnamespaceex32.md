...
description: Installs a specified 32-bit namespace provider. (WSCInstallNameSpaceEx32)
helpviewer_keywords: ["WSCInstallNameSpaceEx32","WSCInstallNameSpaceEx32 function [Winsock]","winsock.wscinstallnamespaceex32","ws2spi/WSCInstallNameSpaceEx32"]
...
The 
**WSCInstallNameSpaceEx32** function installs a specified 32-bit namespace provider. For providers that are able to support multiple names spaces, this function must be called for each namespace supported, and a unique provider identifier must be supplied each time.
<div class="alert">**Note**  This call is a strictly 32-bit version of <a href="/windows/desktop/api/ws2spi/nf-ws2spi-wscinstallnamespaceex">WSCInstallNameSpaceEx32</a> for use on 64-bit platforms. It is provided to allow 64-bit processes to access the 32-bit catalogs.</div><div> </div>
...
The provider-specific data blob associated with namespace entry
                     passed in the <i>lpProviderInfo</i> parameter can be queried using <a href="/windows/desktop/api/ws2spi/nf-ws2spi-wscenumnamespaceprovidersex32">WSCEnumNameSpaceProvidersEx32</a> function.
