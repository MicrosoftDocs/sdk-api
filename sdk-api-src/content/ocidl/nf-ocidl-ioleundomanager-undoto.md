...
title: IOleUndoManager::UndoTo method
...
ms.date: 3/26/2018
ms.keywords: IOleUndoManager, IOleUndoManager interface [COM], UndoTo method, IOleUndoManager::UndoTo, UndoTo method [COM], UndoTo method [COM], IOleUndoManager interface, UndoTo,IOleUndoManager.UndoTo, _ole_ioleundomanager_undoto, com.ioleundomanager_undoto, ocidl/IOleUndoManager::UndoTo
...
<td width="60%">
Both the undo attempt and the rollback attempt failed. The undo manager should never propagate the E_ABORT obtained from a contained undo unit. Instead, it should map any E_ABORT values returned from other undo units to E_FAIL. The undo manager should map any E_ABORT value returned from other undo units to E_FAIL because the caller of <a href="https://msdn.microsoft.com/49c98126-4b99-449e-b08c-f21f98c7c56a">IOleUndoManager::UndoTo</a> knows that the undo attempt and the rollback attempt failed and this is the only reason for the return value of E_ABORT. 
...
Note that the specified undo unit must be a top-level unit, typically retrieved through <a href="https://msdn.microsoft.com/7199910b-3ea3-4b4e-89df-c1188195941c">IOleUndoManager::EnumUndoable</a>.
...
If the undo manager has called the <a href="https://msdn.microsoft.com/5f087779-ef92-41c9-94e6-61d07d5731a7">Do</a> method on more than one top-level unit, it should only rollback the unit that returned the error. The top-level units that succeeded should not be rolled back.
...
<a href="https://msdn.microsoft.com/7199910b-3ea3-4b4e-89df-c1188195941c">IOleUndoManager::EnumUndoable</a>



<a href="https://msdn.microsoft.com/1d5d0cb6-2c1b-49c8-8923-59845fa6231c">IOleUndoManager::RedoTo</a>
